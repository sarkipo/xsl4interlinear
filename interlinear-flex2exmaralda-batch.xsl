...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    ...
    <!--
            ...
            Based on interlinear-eaf2exmaralda.xsl v1.11.
            v1.20: batch version separated from single file version.
    -->

    <xsl:param name="folder" select="'file:///D:/YandexDisk/L-DIGITIZING/EXMARALDA-HAMBURG/toDO/'"/>
    ...
    <xsl:template name="main">
        <xsl:for-each select="$sourcecollection">
            ...
            <xsl:result-document method="xml" indent="yes" encoding="utf-8" omit-xml-declaration="no"
                href="{concat($folder,$filename,'.exb')}">
                <basic-transcription>
                    <head>
                        <meta-information>
    ...
    <xsl:function name="my:word" as="xs:boolean">
        <xsl:param name="word"/>
        <!-- as="element(word)" -->
        <xsl:value-of select="if ($word/item/@type='punct' and $word/item/@lang='nio-x-lat' and not(matches($word/item[@type='punct'],'(///)|(\?\?\?)|(…)|(\.\.\.)'))) then false() else true()"/>
    </xsl:function>
