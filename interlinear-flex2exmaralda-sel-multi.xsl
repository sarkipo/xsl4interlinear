...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    ...
    <xsl:variable name="tiers-sent">ref ts fr src srl nt</xsl:variable>
    ...
    <xsl:template match="/">
        <conversion-report filename="{$filename}">
            <settings>
                <filename><xsl:value-of select="$settings-file"/></filename>
                <setting><xsl:value-of select="$settings//setting[1]"/></setting>
            </settings>
    ...
    <xsl:template match="//interlinear-text">
            ...
            <xsl:variable name="speaker" select="if(substring-before($textname,'_') = '') then 'SPK' else substring-before($textname,'_')"/>
            ...
            <basic-body>
                ...
                <xsl:call-template name="tier-sent-join">
                    ...
                    <xsl:with-param name="lang" select="'sel'"/>
...
<!--                <xsl:call-template name="tier-sent">
    ...
    <xsl:function name="my:word" as="xs:boolean">
        <xsl:param name="word"/>
        <!-- as="element(word)" -->
        <xsl:value-of select="if ($word/item/@type='punct' and $word/item/@lang='sel' and not(matches($word/item[@type='punct'],'(///)|(\?\?\?)|(…)|(\.\.\.)'))) then false() else true()"/>
    </xsl:function>
