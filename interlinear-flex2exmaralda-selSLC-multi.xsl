...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    ...
    <!--
            This is the transform from SIL FLEx flextext format into EXMARaLDA exb, tuned for Selkup project (Donner's texts).
    ...
    <xsl:variable name="tiers-sent">ref ts fr fg nt</xsl:variable>
    ...
    <xsl:param name="settings-file" select="'settings.xml'"></xsl:param>
    <xsl:variable name="settings" select="doc($settings-file)"/>
    <xsl:variable name="baseline-ws" select="$settings//tiers/baseline/@ws"/>
    ...
    <xsl:template match="//interlinear-text">
            ...
            <xsl:variable name="textname" select="current()/item[@type='title' and @lang='en']/text()"/>
            <xsl:variable name="speaker" select="if(substring-before($textname,'_') = '') then 'SPK' else 
                if ($settings//speaker-code/@position = '2') then substring-before(substring-after($textname,'_'),'_') else substring-before($textname,'_')"/>
            ...
            <basic-body>
                ...
                <xsl:call-template name="tier-sent-join">
                    <!-- ts: Complete sentence text (joined from tx words) -->
                    <xsl:with-param name="itemtype" select="'txt'"/>
                    <xsl:with-param name="lang" select="$baseline-ws"/>
                    <xsl:with-param name="cat" select="'ts'"/>
                ...
                <xsl:call-template name="tier-morph">
                    ...
                    <xsl:with-param name="speaker" select="$speaker"/>
                </xsl:call-template><!--<xsl:call-template name="tier-morph">
                    <!-\- go -\->
                    <xsl:with-param name="itemtype" select="'gls'"/>
                ...
                <xsl:call-template name="tier-sent">
                    <!-- fg: German free translation -->
                    <xsl:with-param name="itemtype" select="'gls'"/>
                    <xsl:with-param name="lang" select="'de'"/>
                    <xsl:with-param name="cat" select="'fg'"/>
                    <xsl:with-param name="display" select="'fg'"/>
                    <xsl:with-param name="speaker" select="$speaker"/>
                </xsl:call-template>
<!--                <xsl:call-template name="tier-sent">
                    <!-\- src: Source Cyrillic transcription  -\->
                    <xsl:with-param name="itemtype" select="'lit'"/>
                    <xsl:with-param name="lang" select="'sel-Cyrl-x-source'"/>
                    <xsl:with-param name="cat" select="'src'"/>
                    <xsl:with-param name="display" select="'src'"/>
                    <xsl:with-param name="speaker" select="$speaker"/>
                </xsl:call-template>
                <xsl:call-template name="tier-sent">
                    <!-\- src: Source Latin transcription  -\->
                    <xsl:with-param name="itemtype" select="'lit'"/>
                    <xsl:with-param name="lang" select="'sel-Latn-x-source'"/>
                    <xsl:with-param name="cat" select="'srl'"/>
                    <xsl:with-param name="display" select="'srl'"/>
                    <xsl:with-param name="speaker" select="$speaker"/>
                </xsl:call-template>-->
                <xsl:call-template name="tier-sent">
                    <!-- nt: Comments (notes) -->
    ...
    <xsl:template name="tier-sent-join">
        <!-- from flextext -->
        <xsl:param name="itemtype"/>
        <xsl:param name="lang" select="''"/>
        <!-- for exmaralda -->
        ...
        <tier id="{$display}" speaker="{$speaker}" category="{$cat}" type="{$type}" display-name="{$display}">
            <xsl:for-each select=".//phrase">
                ...
                <xsl:for-each-group select=".//word"
                    ...
                    <!-- WHEN SENTENCE STARTS WITH PUNCTUATION, IT IS STICKED TO THE FIRST WORD -->
                    <event start="{concat('T',$ts-start+position()-1)}" end="{concat('T',$ts-start+position())}">
                        <xsl:value-of select="current-group()/item[@type=$itemtype]" separator=""/>
                    </event>
                </xsl:for-each-group>
            </xsl:for-each>
        </tier>
    ...
    <xsl:function name="my:word" as="xs:boolean">
        <xsl:param name="word"/>
        <!-- as="element(word)" -->
        <xsl:value-of select="if ($word/item/@type='punct' and $word/item/@lang=$baseline-ws and not(matches($word/item[@type='punct'],'(///)|(\?\?\?)|(…)|(\.\.\.)'))) then false() else true()"/>
    </xsl:function>
